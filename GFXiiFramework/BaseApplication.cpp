...
int BaseApplication::Run()
{
	...
	while ( !m_terminate )
	{
		...
		//Logic (we need some way of checking the time between frames so that doing stuff at 10 frames is the same as 93853813 frames
		m_appwnd->Update();
...
LRESULT CALLBACK BaseApplication::WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	switch ( msg )
	{
		...
		case WM_MOUSEMOVE:
			...
			Input::Instance()->UpdateMousePosition(GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
			break;

		case WM_LBUTTONUP:
			s_oglapp->GetApplicationWindow()->MouseLBUp( GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam) );
			Input::Instance()->ToggleKeyState(wparam, true);
			break;
		...
		case WM_KEYUP:
			Input::Instance()->ToggleKeyState(wparam, false);
			break;
