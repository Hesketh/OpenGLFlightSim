...
layout (location = 5) uniform mat4 model;		//modeltoworld matrix
layout (location = 6) uniform mat4 view;		//view matrix
...
{	
	...
	mat4 normalMatrix = transpose(inverse(model)); //only seems to work for the plane and coin. Problems for terrain + house???
	outNormal = normalMatrix * inNormal; 

	lightvec = lightpos - model*position;
